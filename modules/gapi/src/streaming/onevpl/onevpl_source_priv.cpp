...
OneVPLSource::Priv::Priv(std::shared_ptr<IDataProvider> provider, const std::vector<oneVPL_cfg_param>& params) :
    OneVPLSource::Priv()
{
    // Enable Config
    if (params.empty())
    ...
    try {
        ...
        try {
            ...
            if (!engine) {
                ...
                {
                    GAPI_Assert(false &&
                                "OneVPLSource mfx_impl_desription->ApiVersion.Major >= VPL_NEW_API_MAJOR_VERSION"
                                " - is not implemented");
                }
                else
                {
...
const std::vector<oneVPL_cfg_param>& OneVPLSource::Priv::getDefaultCfgParams()
{
...
const std::vector<oneVPL_cfg_param>& OneVPLSource::Priv::getCfgParams() const
{
    return cfg_params;
}

bool OneVPLSource::Priv::pull(cv::gapi::wip::Data& data)
{
